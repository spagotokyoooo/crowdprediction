...
- LINE通知文のプレビュー
- 手動イベント登録のデモ
- 21:15のLINE振り返り通知と、ランチ・午後・夜ごとの3択フィードバック
- LINE Webhookの署名検証、重複イベント抑止、「今日／明日／今週」への応答
- スケジューラから呼べる朝の予報通知・夜の振り返り通知エンドポイント
...
5. 同じスケジューラから、月〜土の21:15（Asia/Tokyo）に `POST /api/jobs/evening-feedback` を呼ぶ。LINEでランチ・午後・夜の実績を順に聞きます（日曜は自動で送信しません）。

ローカルで通知文だけ確認する場合は `GET /api/line/preview`、週間版は `GET /api/line/preview?period=week`、振り返り通知は `GET /api/line/feedback-preview` を利用できます。実際のLINE送信は、アクセストークンと通知先IDを設定するまで実行されません。

振り返りの回答は、現段階ではアプリのローカルファイルに記録します。そのためRenderの再デプロイ・再起動後も継続して学習に使うには、次の段階でデータベースを接続します。
